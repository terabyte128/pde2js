...
public class JavaScriptPrinterVisitor implements VoidVisitor<Void> {
    ...
    public void visit(final FieldDeclaration n, final Void arg) {
        ...
        // JS doesn't require writing down type names
        printer.print("var");
        ...
        for (final Iterator<VariableDeclarator> i = n.getVariables().iterator(); i.hasNext(); ) {
            ...
            // do not assign the variable, just declare it
            var.getName().accept(this, arg);
            if (i.hasNext()) {
                printer.print(", ");
            }
        }
    ...
    public void visit(final BlockStmt n, final Void arg) {
        ...
        List<FieldDeclaration> fieldDeclarations = new NodeList<>();

        // check whether the parent node is the "setup()" method so we know to assign class variables
        if (n.getParentNode().isPresent()
                && n.getParentNode().get() instanceof MethodDeclaration
                && ((MethodDeclaration) n.getParentNode().get()).getNameAsString().equals("setup")
                && n.getParentNode().get().getParentNode().isPresent()
                && n.getParentNode().get().getParentNode().get() instanceof ClassOrInterfaceDeclaration) {

            // collect all of the class fields
            NodeList<BodyDeclaration<?>> allNodes = ((ClassOrInterfaceDeclaration) n.getParentNode().get().getParentNode().get()).getMembers();

            for (BodyDeclaration<?> declaration : allNodes) {
        ...
        if (n.getStatements() != null || !fieldDeclarations.isEmpty()) {
            printer.indent();

            // assign class fields before doing anything else
            for (FieldDeclaration declaration : fieldDeclarations) {
                for (final VariableDeclarator var : declaration.getVariables()) {
                    // only initialize variables that would have been initialized as fields
